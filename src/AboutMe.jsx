...
const REVEAL_CONTENT = [
  {
    upper: ["Aryan Saini", "B.Tech Computer Science (AI)", "ABES Institute of Technology"],
    lower: "Ghaziabad, India • Batch 2023-2027",
  },
  {
    upper: [
      "Freelance Developer (Self-Employed) • 2024-Present",
      "AI/ML Intern at FireLLama Tech",
      "25K+ Instagram Followers Personal Brand",
    ],
    lower: "Designed CRAG systems & Edge AI models",
  },
  {
    upper: [
      "AI Health Voice Agent (Hacknovate 7.0 Winner)",
      "Floatchat AI (SIH 2025 Shortlisted)",
      "Verichain (Blockchain Property Transfer Finalist)",
    ],
    lower: "Python, LangChain, React, Web3 Specialist",
  },
];
...
const ITEMS = [
  {
    id: "identity", label: "IDENTITY", icon: "👤", barIcon: icon1, bars: 1, newBars: [0], counts: ["LVL 20"],
    links: ["Profile Data"],
    stats: [
      { tag: "EXP", value: "3+",  color: "#00a0dc" },
      { tag: "AGE", value: "20",  color: "#f6b300" },
    ],
  },
  {
    id: "journey", label: "JOURNEY", icon: "⚔️", barIcon: icon2, bars: 1, newBars: [0], counts: ["ACTIVE"],
    links: ["Experience Log"],
    stats: [
      { tag: "FOL", value: "25K+", color: "#4078c0" },
      { tag: "INT", value: "2",  color: "#c4001a" },
    ],
  },
  {
    id: "achievements", label: "ACHIEVEMENT", icon: "🏆", barIcon: icon3, bars: 1, newBars: [0], counts: ["UNLOCKED"],
    links: ["Hall of Fame"],
    stats: [
      { tag: "PRJ", value: "10+", color: "#ff6b6b" },
      { tag: "AWD", value: "4x",  color: "#4ecdc4" },
    ],
  },
];
  ...
  return (
    <div id="menu-screen">
      ...
      <div style={{
        ...
        <LoopingVideo src={rainVideo} opacity={1} />
      </div>
      ...
      {revealed && (
        <div key={`nav-${active}`} className="sc-right-nav">
          <span 
            ...
            onClick={() => { window.playPersonaSound?.('hover'); setActive(i => Math.max(0, i - 1)); }}
          >◄</span>
          <span 
          ...
          <span 
            className="sc-nav-btn"
            style={{ cursor: 'pointer', pointerEvents: 'all' }}
            onClick={() => { window.playPersonaSound?.('hover'); setActive(i => Math.min(ITEMS.length - 1, i + 1)); }}
          >RB</span>
          <span 
      ...
      <style>{`
        ...
        .sc-right-nav {
          ...
          align-items: center;
          gap: 6px;
          pointer-events: none; /* children have pointer-events: all */
          z-index: 14;
          transform: translateX(-40px) rotate(-20deg);
          transform-origin: left bottom;
      ...
      <div className="sc-root" role="navigation">
        {ITEMS.map((item, i) => (
          <div
            ...
            onMouseEnter={() => {
              if (active !== i) window.playPersonaSound?.('hover');
              setActive(i);
            }}
          >
